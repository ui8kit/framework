...
type Command = "generate" | "sync" | "scaffold" | "install" | "all";
...
function parseCommand(): Command {
  ...
  if (["generate", "sync", "scaffold", "install", "all"].includes(cmd)) {
...
async function resolveRuntimeConfig(): Promise<{ target: string; domain: Domain }> {
...
async function main(): Promise<void> {
  ...
  const { target, domain } = await resolveRuntimeConfig();
