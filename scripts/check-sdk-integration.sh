...
if node -e "const fs=require('fs');const roots=['apps/engine/$ENGINE_OUT'];const needles=['<If','<Loop','<Var','@ui8kit/dsl','data-gen-'];let bad=false;const walk=(d)=>{for(const e of fs.readdirSync(d,{withFileTypes:true})){const f=d+'/'+e.name;if(e.isDirectory())walk(f);else{const t=fs.readFileSync(f,'utf8');if(needles.some(n=>t.includes(n))){bad=true;}}}};for(const r of roots){walk(r);}process.exit(bad?0:1);"; then
