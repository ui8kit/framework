...
export class HtmlConverter {
  ...
  /**
   * Deduplicate CSS rules by combining selectors with identical content
   */
  private deduplicateCssRules(cssRules: string[]): string[] {
    // Group rules by their content (properties/declarations)
    const contentToSelectors = new Map<string, string[]>();

    for (const rule of cssRules) {
      // Parse the rule: "selector { content }" -> ["selector", "content"]
      ...
      // Skip rules with unknown classes comments (they're unique anyway)
    ...
    // Rebuild rules, combining selectors with identical content
    const deduplicatedRules: string[] = [];

    for (const [content, selectors] of contentToSelectors.entries()) {
      if (selectors.length > 1) {
        // Multiple selectors with same content - combine them
        const combinedSelector = selectors.sort().join(', ');
  ...
  private generateApplyCss(selectorMap: Map<string, string[]>): string {
    ...
    // Deduplicate CSS rules with identical @apply directives
 ...
 */\n\n`;
  ...
  private isValidTailwindClass(className: string): boolean {
    ...
    // Skip Lucide icon classes (they're handled by components, not CSS)
    if (className === 'lucide' || className.startsWith('lucide-')) {
      return false; // Not valid for CSS generation, but don't warn
    }
    ...
    for (const selector of sortedSelectors) {
      ...
      for (const className of classes) {
        ...
        if (cssProperty) {
          ...
          // Skip Lucide icon classes (they're handled by components, not CSS)
          if (className === 'lucide' || className.startsWith('lucide-')) {
            // Don't add unknown class comment for Lucide icons
            continue;
          }
