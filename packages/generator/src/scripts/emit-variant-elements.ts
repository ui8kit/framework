...
function generateElementFile(
  ...
  const baseEntity = entities.find((e) => e.name === filePrefix);
  ...
  const imports = `import { ${componentName} } from "${componentsImportPath}";`;
...
/**
 * Individual ${filePrefix} elements as named exports.
 * Use these for copy-paste or registry integration.
 */
${generateNamedExports(filePrefix, componentName, allVariants, componentsImportPath)}
`;
