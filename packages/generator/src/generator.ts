...
export class Generator {
  ...
  private variantsSelectorsSet: Set<string> | null = null;
  ...
  private async generateHtml(config: GeneratorConfig): Promise<void> {
    ...
    for (const [routePath, route] of Object.entries(config.html.routes)) {
      ...
      try {
        ...
        // Validate semantic tokens early, before stripping/transforming attributes
        this.validateDataClassesTokens(viewContent, routePath);
  ...
  private async emitVariantsApplyCss(config: GeneratorConfig): Promise<string> {
    try {
      ...
      const { emitVariantsArtifacts } = await import('./scripts/emit-variants-apply.js');
      const variantsDir = await this.resolveVariantsDir();
      if (!variantsDir) {
        console.warn('⚠️ variants directory not found (tried src/variants and apps/local/src/variants), skipping variants.apply.css');
        return '';
      }
      const artifacts = await emitVariantsArtifacts({
        variantsDir,
      });
      this.variantsSelectorsSet = new Set(artifacts.selectors);

      await Bun.write(`${config.css.outputDir}/variants.apply.css`, artifacts.css);
      await Bun.write(`${config.css.outputDir}/variants.selectors.json`, JSON.stringify(artifacts.selectors, null, 2));
      await Bun.write(`${config.css.outputDir}/variants.selector-tokens.json`, JSON.stringify(artifacts.selectorToTokens ?? {}, null, 2));

      // A tiny TS helper file that downstream tooling can copy into source if needed.
      const tsTypes = `/* Generated by @ui8kit/generator. */\n` +
        `export const variantsSelectors = ${JSON.stringify(artifacts.selectors, null, 2)} as const;\n` +
        `export type VariantSelector = typeof variantsSelectors[number];\n`;
      await Bun.write(`${config.css.outputDir}/variants.selectors.ts`, tsTypes);

      // Also write strict authoring-time types into the app source tree (if found).
      const appLibDir = await this.resolveAppLibDir();
      if (appLibDir) {
        const generatedPath = join(appLibDir, 'semantic-selectors.generated.ts');
        await this.ensureDir(appLibDir);
        const fileContent =
          `/*\n` +
          ...
          ` */\n\n` +
          `export const semanticSelectors = ${JSON.stringify(artifacts.selectors, null, 2)} as const;\n` +
          `export type SemanticSelector = typeof semanticSelectors[number];\n`;
        await Bun.write(generatedPath, fileContent);
        console.log(`✅ Generated ${relative(process.cwd(), generatedPath)} (${artifacts.selectors.length} selectors)`);
      } else {
        console.warn('⚠️ Could not find app src/lib to write semantic-selectors.generated.ts (tried src/lib and apps/local/src/lib)');
      }

      console.log(`✅ Generated ${config.css.outputDir}/variants.apply.css (${artifacts.css.length} bytes)`);
      console.log(`✅ Generated ${config.css.outputDir}/variants.selectors.json (${artifacts.selectors.length} selectors)`);
      return artifacts.css;
    } catch (error) {
  ...
  private validateDataClassesTokens(html: string, routePath: string): void {
    if (!this.variantsSelectorsSet) return;

    const tokenPattern = /^[a-z][a-z0-9-]*$/;
    const dataClassesRegex = /data-classes\s*=\s*["']([^"']*)["']/g;
    let match: RegExpExecArray | null;

    while ((match = dataClassesRegex.exec(html)) !== null) {
      const raw = match[1] ?? '';
      const tokens = raw.trim().split(/\s+/g).filter(Boolean);
      for (const t of tokens) {
        if (!tokenPattern.test(t)) {
          console.warn(`⚠️ [data-classes] Invalid token "${t}" on route "${routePath}". Allowed: ${tokenPattern}`);
          continue;
        }
        if (!this.variantsSelectorsSet.has(t)) {
          console.warn(`⚠️ [data-classes] Unknown selector "${t}" on route "${routePath}". Not found in variants.selectors.json`);
        }
      }
    }
  }
  ...
  private async resolveAppLibDir(): Promise<string | null> {
    const candidates = [
      // When generator is executed from an app workspace (e.g. apps/local)
      join(process.cwd(), 'src', 'lib'),
      // When executed from monorepo root
      join(process.cwd(), 'apps', 'local', 'src', 'lib'),
    ];
