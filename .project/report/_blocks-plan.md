...
Изучаю структуру существующих пакетов, чтобы следовать установленным паттернам. Затем начну реализацию плана.

Плагины для Liquid и Handlebars уже существуют. Обновляю конфигурацию генератора для поддержки нескольких движков.

Рефакторинг завершен. Резюме выполненной работы:

## Выполненные задачи

### 1. Инфраструктура пакетов
- Создан пакет `@ui8kit/blocks` с полной структурой (blocks, layouts, partials, components)
- Создан пакет `@ui8kit/data` с fixtures и типизацией
- Настроены конфигурации (package.json, tsconfig.json, vite.config.ts)

### 2. DSL соглашения
- Создан DSL guide (`packages/blocks/DSL.md`)
- Определены соглашения по использованию компонентов If, Var, Loop, Slot
- Установлены правила именования `data-class`

### 3. Миграция блоков
- Мигрированы 6 основных блоков: HeroBlock, FeaturesBlock, CTABlock, PricingBlock, TestimonialsBlock, DashboardBlock
- Все блоки используют DSL компоненты из `@ui8kit/template`
- Мигрированы layouts (MainLayout, DashLayout) и partials (Header, Footer, Sidebar)

### 4. Миграция данных
- Созданы JSON fixtures: hero, features, pricing, testimonials, cta, dashboard
- Добавлена типизация для всех fixtures
- Настроены экспорты в `packages/data/src/index.ts`

### 5. Обновление приложений
- Обновлен `apps/web`: добавлены зависимости, обновлен HomePage для использования новых пакетов
- Обновлен `apps/engine`: добавлены зависимости, обновлен main.tsx
- Обновлены generator.config.ts для использования `packages/blocks/src`

### 6. Плагины генератора
- Плагины Liquid и Handlebars уже существовали и работают
- Создана документация `packages/generator/PLUGINS.md`
- Обновлены конфигурации для поддержки генерации из пакетов

### 7. Тестирование
- Добавлены тесты генератора (`apps/engine/test/blocks.test.ts`)
- Настроен vitest для engine приложения
- Тесты проверяют генерацию для Liquid и Handlebars

## Результат

Теперь у вас есть:
- DRY архитектура: один источник правды для блоков в `packages/blocks`
- Консистентность: web и engine используют идентичные компоненты
- Тестируемость: тесты проверяют генерацию шаблонов
- Масштабируемость: легко добавить новые приложения
- Профессионализм: стандартный monorepo паттерн

Все готово для использования. Можно запускать `npm run build` для сборки всех пакетов и `npm run generate` для генерации шаблонов.